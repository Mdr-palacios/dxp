...
def main() -> int:
    ...
    # 1. Local index exists and has 18 chunks across 6 source files.
    ...
    if len(chunks) < 6:
        failures.append(f"expected >=6 chunks, got {len(chunks)}")
    sources = {c["metadata"]["source_file"] for c in chunks}
    if len(sources) != 6:
        failures.append(f"expected 6 distinct source files, got {len(sources)}: {sources}")
    ...
    print("PASS: all 7 assertion groups OK.")
    return 0
